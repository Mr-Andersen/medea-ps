...
import Parsing (fail)
import Parsing.Combinators (lookAhead, manyTill, (<?>))
import Parsing.String (anyChar, char, satisfy, string)
import Parsing.String.Basic (digit, letter, takeWhile1)
...
parseIdentifier = do
  ident <- takeWhile1 (not <<< isSeparatorOrControl) <?> "identifier"
...
parseReserved reserved = do
  ident <- takeWhile1 (not <<< isSeparatorOrControl) <?> "reserved"
  let
